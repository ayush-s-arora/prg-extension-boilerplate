...
const EXTENSION_ID = 'poseBodyMulti';
...
class Scratch3PoseNetBlocks {
    ...
    async ensureBodyModelLoaded() {
        if (!this._bodyModel) {
            ...
            this._bodyModel = await poseDetection.createDetector(moveModel, {modelType: poseDetection.movenet.modelType.MULTIPOSE_LIGHTNING, enableTracking: true});  //enables multi-person detection
    ...
    getInfo () {
        ...
        return {
            id: EXTENSION_ID,
            name: formatMessage({
                id: 'posenetMulti.categoryName',
                default: 'Multi-Person Body Pose Sensing',
            ...
            blocks: [
                {
                    opcode: 'goToPart',
                    text: "go to person [PERSON_INDEX]'s [PART]",
                ...
                },
                {
                    ...
                    text: "x position of person [PERSON_INDEX]'s [PART]",
                ...
                },
                {
                    ...
                    text: "y position of person [PERSON_INDEX]'s [PART]",
                    blockType: BlockType.REPORTER,
                    isTerminal: true,
                    arguments: {
                        PART: {
                ...
                {
                    opcode: 'partAngle',
                    text: "angle between person [PERSON_INDEX]'s [PART], [PART2] and [PART3]",
                    blockType: BlockType.REPORTER,
                    isTerminal: true,
                    arguments: {
                        PART: {
                ...
                {
                    opcode: 'numPeople',
                    text: 'number of people detected',
                    blockType: BlockType.REPORTER,
                    isTerminal: true
                }
    ...
    goToPart(args, util) {
        console.log(this.poseStates);
        if (this.hasPose(args['PERSON_INDEX'])) {
            const pt = this.poseStates[args['PERSON_INDEX']].keypoints.find(point => point.name === args['PART']);
            if (pt) {
                if (pt.score > 0.2) {
    ...
    hasPose(personIndex) {
        return this.poseStates[personIndex] && this.poseStates[personIndex].keypoints && this.poseStates[personIndex].score > 0.01;
    }
    ...
    posePositionX(args, util) {
        ...
        if (this.hasPose(args['PERSON_INDEX'])){
            return this.tfCoordsToScratch({x: this.poseStates[args['PERSON_INDEX']].keypoints.find(point => point.name === args['PART']).x}).x;
        }
    }
    ...
    posePositionY(args, util) {
        if (this.hasPose(args['PERSON_INDEX'])){
            return this.tfCoordsToScratch({y: this.poseStates[args['PERSON_INDEX']].keypoints.find(point => point.name === args['PART']).y}).y;
        }
    }

    partAngle(args, util) {
        if (this.hasPose(args['PERSON_INDEX'])){
            p1 = this.poseStates[args['PERSON_INDEX']].keypoints.find(point => point.name === args['PART']);
            p2 = this.poseStates[args['PERSON_INDEX']].keypoints.find(point => point.name === args['PART2']);
            p3 = this.poseStates[args['PERSON_INDEX']].keypoints.find(point => point.name === args['PART3']);
